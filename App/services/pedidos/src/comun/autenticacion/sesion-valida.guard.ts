 ...
 *
 * ADR-02 pone la validación en el API Gateway. El servicio también verifica:
 * si alguien
...
export class SesionValida implements CanActivate {
  ...
  async canActivate(contexto: ExecutionContext): Promise<boolean> {
    ...
    const [esquema, token, extra] = (peticion.headers.authorization ?? '').split(' ');
    if (esquema !== 'Bearer' || !token || extra !== undefined) {
  ...
  private async verificar(token: string): Promise<SesionVerificada | null> {
    ...
    const { sub, roles, jti, iat, exp } = contenido;
    if (
      typeof sub !== 'string' ||
      !ES_UUID.test(sub) ||
      typeof jti !== 'string' ||
