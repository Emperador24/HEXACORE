...
\textbf{Cambios de esta versión.} Los seis diagramas se regeneraron con
\href{https://github.com/tt-a1i/archify}{Archify}, que los produce a partir de una especificación
\texttt{JSON} versionada en el repositorio. El cambio responde a dos necesidades: (1) mantener las
indicaciones del profesor --- cada caja muestra nombre, tipo/tecnología y una breve descripción de
su responsabilidad, y el antiguo \textbf{Diagrama Dinámico} sigue reemplazado por un
\textbf{Diagrama de Secuencia} UML con líneas de vida, mensajes numerados y barra de activación
---; y (2) que el diagrama deje de desactualizarse respecto al código: al vivir en un archivo de
texto, una decisión como ADR-07 (unificar las dos apps móviles en una sola) se refleja editando la
especificación y regenerando, en vez de reeditando una imagen a mano.

Las fuentes \texttt{JSON} y la versión interactiva (\texttt{HTML} con pan/zoom y tema
claro/oscuro) quedan en \texttt{Work/Diagrams/Archify/}; los PDF que se insertan en este documento,
en \texttt{Work/Diagrams/ArchifyPDF/}. Los archivos \texttt{.drawio} de la versión anterior se
conservan en \texttt{Work/Diagrams/} como referencia histórica, pero \textbf{ya no son la fuente
de los diagramas de este documento}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Diagrams/ArchifyPDF/01-contexto.pdf}
\caption{Diagrama de contexto (Nivel 1, modelo C4). Generado con Archify a partir de una especificación JSON; la versión interactiva
(pan/zoom, tema claro/oscuro) está en \texttt{Diagrams/Archify/}.}
\label{fig:contexto}
\end{figure}

\textbf{Leyenda:} cada figura incluye su propia leyenda de colores, generada por Archify a partir
del tipo de cada caja. En este nivel: verde = el sistema en alcance de este documento; gris =
personas (roles) y sistemas externos, ambos fuera de la frontera del sistema.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Diagrams/ArchifyPDF/02-contenedores.pdf}
\caption{Diagrama de contenedores (Nivel 2, modelo C4). Generado con Archify a partir de una especificación JSON; la versión
interactiva está en \texttt{Diagrams/Archify/}.}
\label{fig:contenedores}
\end{figure}

\textbf{Leyenda:} cian = interfaz de usuario; verde = microservicio; rojo = API Gateway; violeta =
almacenamiento (Redis y bases de datos); naranja = cola de mensajes; gris = sistema externo. Las
flechas punteadas indican interacciones asíncronas.
...
\bottomrule
\endlastfoot
Cliente & App Móvil / Portal Web Cliente & Consulta eventos, compra y revende entradas, reserva parqueadero. & HTTPS\\
Organizador & Portal Web Admin & Crea eventos, asigna personal y coordina la operación. & HTTPS\\
Empleado & App Móvil & Consulta turnos y registra ingreso/salida. & HTTPS\\
Administrador & Portal Web Admin & Administra usuarios, personal y configuración. & HTTPS\\
Supervisor de emergencia & Portal Web Admin & Activa y coordina el protocolo de evacuación. & HTTPS\\
Las tres interfaces & API Gateway & Envía las solicitudes del usuario al backend. & HTTPS/JSON\\
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Diagrams/ArchifyPDF/03-componentes.pdf}
\caption{Diagrama de componentes — Servicio de Entradas y Mercado Secundario (Nivel 3, modelo C4).
Generado con Archify a partir de una especificación JSON; la versión
interactiva está en \texttt{Diagrams/Archify/}.}
\label{fig:componentes}
\end{figure}

\textbf{Leyenda:} verde = componente interno del Servicio de Entradas; gris = contenedor o sistema
de otro nivel, que aquí solo se referencia y no se detalla.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Diagrams/ArchifyPDF/04-panorama.pdf}
\caption{Diagrama de panorama de sistemas (\textit{System Landscape}, modelo C4). Generado con Archify; la versión interactiva está en
\texttt{Diagrams/Archify/}.}
\label{fig:panorama}
\end{figure}

\textbf{Leyenda:} verde = el sistema en alcance de este documento; ámbar = otro sistema de la
organización; gris = sistema externo.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Diagrams/ArchifyPDF/05-secuencia.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.8\textheight,keepaspectratio]{Diagrams/ArchifyPDF/06-despliegue.pdf}
\caption{Diagrama de despliegue (\textit{Deployment}, modelo C4). Generado con Archify; la versión interactiva está en
\texttt{Diagrams/Archify/}.}
\label{fig:despliegue}
\end{figure}

\textbf{Leyenda:} ámbar = nodo de infraestructura (CDN/Edge); verde = clúster de aplicación; violeta
= clústeres de datos; naranja = clúster de mensajería; rojo = balanceador + API Gateway; gris =
dispositivo del usuario y sistemas externos. El recuadro punteado delimita el ambiente de
...
Archify con nombre, tipo/tecnología y descripción en cada caja, y sus fuentes
\texttt{JSON} quedan en \texttt{Work/Diagrams/Archify/} para su regeneración futura. Este documento debe
