...
Gateway. La \textbf{App Móvil} es un único binario (Kotlin + Jetpack Compose) que sirve tanto al
rol Cliente como al rol Personal: el login identifica el rol del usuario autenticado y la
navegación interna abre el conjunto de pantallas correspondiente — no son dos apps distribuidas
por separado (ver ADR-07). El backend está compuesto por microservicios independientes por dominio
...
\textbf{Solución:} una única app (\texttt{App/frontend/app-movil-cliente}\footnote{El nombre del
módulo quedó de la primera versión; se recomienda renombrarlo en una futura iteración para no sugerir
que es exclusivo del rol Cliente.}) con una pantalla de login compartida. Las credenciales
identifican el rol del usuario autenticado y la navegación interna (\textit{Jetpack Navigation
Compose}) abre el conjunto de pantallas del rol correspondiente: Inicio/Entradas/Parqueadero/Pedidos
para Cliente, Turnos/Asistencia para Personal.\\
