...
\node[draw=azul,rounded corners,fill=gris,minimum width=10cm,minimum height=1cm,below=0.4cm of current bounding box.south,align=center]
{Aplicación Móvil: Cliente y Personal};
\end{tikzpicture}
\vfill
\begin{tabular}{rl}
...
Entrega 2 se decidió unificar la Aplicación Móvil del Cliente y la Aplicación Móvil del Personal en
un único binario, con un login compartido que identifica el rol y navega al conjunto de pantallas
correspondiente (ver SAD, ADR-07). Este documento se actualiza en consecuencia; la decisión original
de dos apps separadas queda documentada como alternativa descartada en ese ADR.}
...
Se contemplan tres clientes principales:
...
\item \textbf{Aplicación Móvil (Cliente y Personal):} un único binario cuyo login identifica el rol del
usuario autenticado. Para el rol Cliente, muestra entradas y códigos QR, información y
notificaciones; para el rol Personal, turnos, asistencia, instrucciones, reportes y alertas.
\end{itemize}
...
\subsection{Aplicación Móvil (Cliente y Personal)}

Una sola aplicación construida en \textbf{Flutter}, que desde una única base de código genera los
binarios de Android e iOS. Tiene una pantalla de login compartida: las credenciales identifican el rol
del usuario autenticado y la navegación interna abre el conjunto de pantallas correspondiente, sin ser
dos apps distribuidas por separado.

Para el \textbf{rol Cliente}, permite iniciar sesión, consultar eventos y entradas, visualizar el
código QR de cada entrada, consultar información del evento y recibir notificaciones y alertas. Su
función crítica es facilitar el acceso rápido a los QR durante el ingreso.

Para el \textbf{rol Personal}, permite consultar eventos asignados, turnos, horarios y áreas de
trabajo; confirmar asignaciones; solicitar cambios de turno; registrar ingreso y salida; recibir
instrucciones; reportar incidentes y recibir alertas de emergencia.
...
Media & Desplegabilidad & Cada microservicio debe poder actualizarse de forma independiente y sin interrumpir el evento en curso, que es cuando el sistema no puede detenerse.\\
Media & Integrabilidad & El sistema depende de dos servicios externos (pasarela de pagos y proveedor de notificaciones) que deben poder sustituirse sin reescribir la lógica de negocio.\\
Media & Seguridad física (Safety) & Una falla durante una evacuación (CU-010) tiene consecuencias sobre la integridad de las personas, no solo sobre los datos.\\
Media & Comprobabilidad & Con seis microservicios independientes, cada dominio debe poder probarse aislado para que el equipo pueda avanzar en paralelo.\\
...
Los cuatro atributos añadidos en esta versión (Desplegabilidad, Integrabilidad, Seguridad física
y Comprobabilidad) provienen de los temas de las clases 6--14 y no estaban en la priorización
inicial del proyecto; se incorporan porque el análisis de tácticas de la sección
\textit{``Cómo la arquitectura satisface los atributos de calidad''} los cubre y porque tienen
requisitos no funcionales asociados (RNF-15 a RNF-18 en \textit{DescripcionArquitecturaSoftware.tex}).

\section{Arquitectura propuesta}

Se propone una arquitectura distribuida basada en \textbf{microservicios}, con API Gateway, balanceador de carga, Redis, RabbitMQ y bases de datos independientes por dominio.
...
\node[box,right=6mm of wa] (am) {App Móvil\\(Cliente + Personal)};

\node[box,below=10mm of $(wc.south)!0.5!(am.south)$] (gw) {API Gateway\\Auth + permisos};
...
\node[infra,below=13mm of $(ms.south)!0.5!(em.south)$] (inf) {Redis \quad | \quad RabbitMQ \quad | \quad Bases de datos};
\node[ext,below=of inf,xshift=-28mm] (pg) {Pasarela de pagos};
\node[ext,below=of inf,xshift=28mm] (no) {Notificaciones\\Push / Email / SMS};

\foreach \x in {wc,wa,am} \draw[arr] (\x)--(gw);
\draw[arr](gw)--(lb);
...
\node[box,right=9mm of e] (q) {RabbitMQ};
\node[box,right=9mm of q] (m) {App Móvil\\(Cliente + Personal)};
\node[box,right=9mm of m] (n) {Notificaciones};
\draw[arr](s)--(e);
\draw[arr](e)--(q);
\draw[arr](q)--(m);
\draw[arr](q)--(n);
...
\midrule
Entradas & Web Cliente, App Móvil & Compra, disponibilidad, validación y gestión.\\
Mercado Secundario & Web Cliente & Publicación, compra y transferencia.\\
Personal & Web Administrativo, App Móvil & Empleados, turnos, asignaciones y asistencia.\\
Eventos & Web Administrativo, Web Cliente, App Móvil & Gestión y consulta de eventos.\\
Parqueaderos & Web Cliente, Web Administrativo, App Móvil & Reservas, espacios, cobros y ocupación.\\
Emergencias & Web Administrativo, App Móvil & Activación, coordinación, alertas y seguimiento.\\
\bottomrule
\end{longtable}
...
RabbitMQ & Procesamiento asíncrono & Rendimiento, escalabilidad\\
...
La arquitectura propuesta conserva las decisiones fundamentales de la primera versión, pero las adapta a una definición más concreta del sistema: un portal web para clientes, un portal web administrativo y operativo, y una aplicación móvil única para clientes y personal, que distingue el rol mediante el login.
